...
import { Calendar, Clock, MapPin, Users, Phone, Info, Image as ImageIcon, Video, FileText } from 'lucide-react';
...
export const Step4Preview = ({
    ...
    const formatDate = (dateStr: string) => {
        if (!dateStr) return '—';
        try {
            return format(new Date(dateStr), isZh ? 'yyyy年MM月dd日 HH:mm' : 'MMM d, yyyy HH:mm', { locale: dateFormatLocale });
        } catch (e) {
            return dateStr;
        }
    };
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            {values.slogan && (
                                <p className="text-stone-500 italic mt-1 font-medium italic">"{values.slogan}"</p>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="flex items-start gap-2">
                                ...
                                <div>
                                    ...
                                    <span className="text-sm font-medium">{formatDate(values.startTime)}</span>
                                </div>
                            </div>
                            {values.endTime && (
                                <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                        ...
                                        <span className="text-sm font-medium">{formatDate(values.endTime)}</span>
                                    </div>
                                </div>
                            )}
                        </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex gap-3 items-start">
                ...
                <p className="text-sm text-amber-800 leading-relaxed">
                    {isZh
                        ? '请仔细检查以上信息。提交后，计划将进入审核队列。在管理员批准之前，您将无法再次编辑此计划。'
                        : 'Please review the information above carefully. Once submitted, your plan will enter the review queue. You will not be able to edit it again until it is approved by an administrator.'}
                </p>
            </div>
        </div>
    );
