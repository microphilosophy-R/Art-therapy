      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm mb-6">
          <div className="flex items-start justify-between gap-4">
            ...
            {form.status !== 'ARCHIVED' && (
              <Button variant="outline" size="sm" onClick={() => archiveMutation.mutate()} loading={archiveMutation.isPending}>
                <Archive className="h-4 w-4" /> {t('forms.detail.archive')}
              </Button>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mt-5 pt-5 border-t border-stone-100">
            <div>
              <p className="text-xs text-stone-400">{t('forms.detail.recipient')}</p>
            ...
            <div>
              <p className="text-xs text-stone-400">{t('forms.detail.sent')}</p>
              <p className="text-sm font-medium text-stone-700 mt-0.5">
                {form.sentAt ? new Date(form.sentAt).toLocaleDateString() : '—'}
              </p>
            </div>
            <div>
              <p className="text-xs text-stone-400">{t('forms.detail.submitted')}</p>
              <p className="text-sm font-medium text-stone-700 mt-0.5">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl border border-stone-200 p-4 shadow-sm flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-stone-400">{t('forms.detail.questions')}</p>
          ...
          <div className="bg-white rounded-xl border border-stone-200 p-4 shadow-sm flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-stone-400">{t('forms.detail.responses')}</p>
          ...
          <div className="bg-white rounded-xl border border-stone-200 p-4 shadow-sm flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-stone-400">{t('forms.detail.status')}</p>
        ...
        {responses.length === 0 ? (
          <div className="bg-white rounded-2xl border border-stone-200 p-10 text-center shadow-sm">
            ...
            <p className="text-stone-500 font-medium">{t('forms.detail.awaitingResponse')}</p>
            <p className="text-stone-400 text-sm mt-1">{t('forms.detail.awaitingDesc')}</p>
          </div>
        ) : (
          <div className="space-y-4">
            <h2 className="text-base font-semibold text-stone-900">{t('forms.detail.clientResponses')}</h2>
            {responses.map((resp, rIdx) => (
              <div key={resp.id} className="bg-white rounded-xl border border-stone-200 p-5 shadow-sm">
                <p className="text-xs text-stone-400 mb-4">
                  {t('forms.detail.submission', { n: rIdx + 1 })} — {new Date(resp.submittedAt).toLocaleString()}
                </p>
                <div className="space-y-4">
                    ...
                    return (
                      <div key={q.id} className="border-b border-stone-100 pb-4 last:border-0 last:pb-0">
                        ...
                        <p className="text-sm text-stone-800">
                          {answer ? renderAnswerValue(q, answer.value) : <span className="text-stone-400 italic">{t('forms.detail.notAnswered')}</span>}
                        </p>
                      </div>
                    );
                  })}
        ...
        {responses.length === 0 && questions.length > 0 && (
          <div className="mt-6 space-y-3">
            <h2 className="text-base font-semibold text-stone-900">{t('forms.detail.formQuestions')}</h2>
            {questions.map((q, i) => (
              <div key={q.id} className="bg-white rounded-xl border border-stone-200 px-4 py-3 text-sm shadow-sm">
                ...
                {q.required && <span className="ml-2 text-xs text-rose-400">{t('common.required')}</span>}
